...
/*   Updated: 2017/12/21 08:55:17 by pchadeni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_file	*part(t_file *h, t_file *end, t_file **nhead, t_file **nend)
{
	...
	while (cur != pivot)
	{
		if (cur->stat.st_ctime > pivot->stat.st_ctime ||
			(cur->stat.st_ctime == pivot->stat.st_ctime &&
			cur->stat.st_ctimespec.tv_nsec >= pivot->stat.st_ctimespec.tv_nsec))
		{
...
void			qs_tstatus(t_file **headref)
{
