...
/*   Updated: 2018/01/02 16:03:44 by pchadeni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_file	*part(t_file *h, t_file *end, t_file **nhead, t_file **nend)
{
	...
	while (cur != pivot)
	{
		if (cur->stat.st_birthtime > pivot->stat.st_birthtime ||
			(cur->stat.st_birthtime == pivot->stat.st_birthtime &&
			cur->stat.BIRTH_NSEC >= pivot->stat.BIRTH_NSEC))
		{
