...
/*   Updated: 2017/12/29 16:50:34 by pchadeni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_file	*part(t_file *h, t_file *end, t_file **nhead, t_file **nend)
{
	...
	while (cur != pivot)
	{
		if (cur->stat.st_mtime > pivot->stat.st_mtime ||
			(cur->stat.st_mtime == pivot->stat.st_mtime &&
			cur->stat.st_mtimespec.tv_nsec >= pivot->stat.st_mtimespec.tv_nsec))
		{
